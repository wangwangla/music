package com.example.learnandroid.constant;

import com.example.learnandroid.data.ContentResolverFindMusic;

public class Constant {
    public static final int INITBOTTOMVIEW = 0;
    public static final String MUSIC_KEY = "MUSIC_KEY";
    public static final String UP_DATE_BOTTOM = "UP_DATE_BOTTOM";
    public static int playStyle = 0;//0 顺序   1 随机
    public static int playRanadom = 0;//0 顺序   1 随机
    public static int playOrder = 1;//0 顺序   1 随机

    public static String MUSIC_PRE = "MUSIC_PRE";
    public static String MUSIC_NEXT = "MUSIC_NEXT";
    public static String MUSIC_STOP = "MUSIC_STOP";
    public static String MUSIC_TYPE = "MUISC_LEARN";

    public static int NO_SET = 0;
}
